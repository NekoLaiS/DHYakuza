...
init python:
    ...
    def screenshot_srf():
        if renpy.version_tuple > (7, 3, 5, 606):
            srf = renpy.display.draw.screenshot(renpy.game.interface.surftree) ## Thanks elckarow to Fix
        else:
...
screen invert(length, delay=0.0):
    add Invert(delay) size (1920, 1080)
...
image m_rectstatic:
    RectStatic(Solid("#000"), 48, 48, 48).sm
    pos (0, 0)
    size (48, 48)
...
init python:
    ...
    class RectStatic(object):
        def __init__(self, theDisplayable, numRects=12, rectWidth = 45, rectHeight = 45):
        ...
        def add(self, d):
            s = self.sm.create(d)
            s.x = random.randint(0, 60) * 48
            s.y = random.randint(0, 35) * 48
...
image bsod = LiveComposite((1920, 1080), (0, 0), "bsod_1", (0, 0), "bsod_2")
